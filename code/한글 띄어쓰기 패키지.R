# 한글 띄어쓰기 패키지 url : https://hwangknock.tistory.com/8
install.packages("reticulate")
library(reticulate)
install_miniconda()
remotes::install_github("haven-jeon/KoSpacing",force = TRUE)
library(KoSpacing)
# 다운로드하고 맨 처음에만 set_env()할것
#set_env()

duplicated_data <- join_data_2[duplicated(join_data_2$EMD_CD),]
duplicated_data_3 <- join_data_2[duplicated(join_data_2$EMD_CD),]
join_data_3 <- join_data_2[-as.numeric(rownames(duplicated_data_3)),]
...
test <- join_data_2
for(i in c(1:length(test$법정동명))){
  if(is.na(test$X.1)[i] == TRUE){
    test$X.1[i] <- test$법정동명[i]
  }
}

test <- test[,-c(5,6)]
duplicated_data_2 <- duplicated_data[,c(2,5,7)]
test2 <- inner_join(test,duplicated_data_2, by = "법정동명")
test3 <- test2[duplicated(test2$EMD_CD),]
as.numeric(rownames(test3))
test4 <- test2[-as.numeric(rownames(test3)),]
test4 <- test4[,-5]


join_data_3[join_data_3$법정동명 %in% test4$법정동명,]$비행불가능여부 <- test4$비행불가능여부
str(join_data_3[is.na(join_data_3$비행불가능여부),])


library(writexl)
write_xlsx(join_data_2,"검토.xlsx")
